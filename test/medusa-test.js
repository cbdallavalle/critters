...
describe('Medusa', function() {
  ...
  it('should unstone the the first victim and stone the 4th coming in', function() {
    // create medusa
    var medusa = new Medusa('Soup');
    // create first victim
    var tomatoVictim = new Person('Tomato');
    // create second victim
    var lettuceVictim = new Person('Lettuce');
    // create third victim
    var carrotVictim = new Person('Carrot');
    // create fourth victim
    var potatoVictim = new Person('Potato');

    // medusa stares at first victim
    medusa.stare(tomatoVictim);
    // assert that the first victim is stoned
    assert.isTrue(tomatoVictim.stoned)
    // medusa stares at second victim
    medusa.stare(lettuceVictim);
    // medusa stares at third victim
    medusa.stare(carrotVictim);
    // medusa stares at fourth victim
    medusa.stare(potatoVictim);

    // assert the fourth victim is stoned
    assert.isTrue(potatoVictim.stoned)
    // assert that the first victim is not stoned
    assert.isFalse(tomatoVictim.stoned)
